...
<form  action="{{url('/guanliyuan/store')}}" method="post" class="form-horizontal" role="form" enctype="multipart/form-data">
	@csrf
	<center><b style="color:red">{{session('errors')}}</b></center>
	<div class="form-group">
		<label for="firstname" class="col-sm-2 control-label">管理员账号</label>
		<div class="col-sm-8">
			<input type="text" class="form-control" name="guan_name" id="firstname" 
				   placeholder="请输入名字">
		</div>
	</div>
	<div class="form-group">
		...
		<div class="col-sm-8">
			<input type="password" class="form-control" name="guan_pwd" id="firstname" 
				   placeholder="请输入年龄">
		</div>
	</div>
	<div class="form-group">
