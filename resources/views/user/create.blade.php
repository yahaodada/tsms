...
<form  action="{{url('/user/store')}}" method="post" class="form-horizontal" role="form" enctype="multipart/form-data">
	...
	<div class="form-group">
		...
		<div class="col-sm-8">
			<input type="password" class="form-control" name="user_pwd" id="optionsRadios1" 
				   placeholder="请输入管理员密码">
			<b style="color:red">{{$errors->first('cate_name')}}</b>	   
		</div>
	</div>
	<div class="form-group">
		<label for="firstname" class="col-sm-2 control-label">确定密码</label>
		<div class="col-sm-8">
			<input type="password" class="form-control" namee="user_pwds" id="optionsRadios1" 
				   placeholder="请确定密码">
			<b style="color:red">{{$errors->first('cate_name')}}</b>
		</div>
	</div>
